...
# Updater: J. Davidson                              #
# Purpose: Added error checking to user inputs.     #
# ################################################# #
# Updated:
# Updater:
# Purpose:
...
def addField(pathway1, gdb1):
    ...
    while True:
        ...
        if noFields.isdigit():
            ...
            print "The number you enter must be passable as an integer. Please enter a number."
...
if __name__ == '__main__':
    ...
    while True:
        ...
        if os.path.isdir(pathway1) and pathway1[-4:].upper() == '.GDB':
            arcpy.env.workspace = pathway1
            break
        else:
            print 'That is not a correct path. Please enter the path to the file GDB.'
            print '\te.g. C:\File.gdb'
    while True:
        gdb1 = raw_input("Please enter the name of the feature class you would like to edit: ")
        lista = arcpy.ListFeatureClasses()
        if not gdb1 in lista:
            print 'That feature class does not exist. Please enter the name of the feature class exactly.'
        else:
            break
